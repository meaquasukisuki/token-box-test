...
import ETHLEVERAGEABI from 'index-sdk/abi/eth2XLeverageToken.json'
...
import { getBoxTokenContract } from 'index-sdk/boxStaking'
...
export const approve = async (
  ...
  try {
    ...
    const tokenContract = contract ? contract : getBoxTokenContract(provider)
...
export const getEth2XleverageTokenContract = (provider: provider, address: string) => {
  const web3 = new Web3(provider)
  const contract = new web3.eth.Contract(
    ETHLEVERAGEABI.abi as unknown as AbiItem,
    address
  )
  return contract
}
